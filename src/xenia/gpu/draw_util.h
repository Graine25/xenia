...
union ResolveEdramInfo {
  ...
  struct {
    ...
    // Flag from gamma_decode_pwl_resolve in resolve shader. Full color resolves
    // decode PWL gamma to linear before MSAA averaging / conversion, then write
    // gamma bytes again for gamma dests. Off keeps the old byte averaging.
