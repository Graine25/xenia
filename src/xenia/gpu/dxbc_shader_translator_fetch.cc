  ...
  if (instr.opcode == FetchOpcode::kGetTextureWeights) {
    ...
    if (instr.opcode == FetchOpcode::kTextureFetch) {
      ...
      dxbc::Src integer_scale_bits_packed = LoadSystemConstant(
          ...
          dxbc::Src::kXXXX);
      // Uniform early-out: the packed scale bits are zero for every fetch
      // except integer num_format fixed formats, and zero means a multiply by
      // 1. The constant is the same for the whole draw, so this branch is
      // coherent - skip the per-component scale work in the common case.
