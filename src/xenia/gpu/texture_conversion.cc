...
void ConvertTexelCTX1ToR8G8(xenos::Endian endian, void* output,
  ...
  CopySwapBlock(endian, block.data, input, sizeof(block.data));

  uint8_t cr[4] = {
      block.r0, block.r1,
      static_cast<uint8_t>(2.0f / 3.0f * block.r0 + 1.0f / 3.0f * block.r1),
      static_cast<uint8_t>(1.0f / 3.0f * block.r0 + 2.0f / 3.0f * block.r1)};
  uint8_t cg[4] = {
      block.g0, block.g1,
      static_cast<uint8_t>(2.0f / 3.0f * block.g0 + 1.0f / 3.0f * block.g1),
      static_cast<uint8_t>(1.0f / 3.0f * block.g0 + 2.0f / 3.0f * block.g1)};
  ...
  for (uint32_t oy = 0; oy < 4; ++oy) {
    for (uint32_t ox = 0; ox < 4; ++ox) {
      ...
      output_bytes[oy * length + ox * 2] = cr[xx];
...
void ConvertTexelDXT3AToDXT3(xenos::Endian endian, void* output,
                             const void* input, size_t length) {
  auto output_bytes = static_cast<uint8_t*>(output);
  CopySwapBlock(endian, output_bytes, input, 8);
  std::memset(output_bytes + 8, 0, 8);
}

// Based on the Xbox 360 2D tiled address mapping.
...
void Untile(uint8_t* output_buffer, const uint8_t* input_buffer,
  ...
  for (uint32_t y = 0; y < untile_info->height; y++) {
    uint32_t input_row_offset = TiledOffset2DRow(
