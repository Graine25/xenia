...
TextureCache::Texture* TextureCache::AllocateTexture(
  ...
  VkResult image_props_status = vkGetPhysicalDeviceImageFormatProperties(
      *device_, format, image_info.imageType, image_info.tiling,
      image_info.usage, image_info.flags, &image_props);
  if (image_props_status != VK_SUCCESS) {
    XELOGE(
        ...
        static_cast<uint32_t>(image_info.flags));
    return nullptr;
  }

  if (image_props.maxExtent.width < image_info.extent.width ||
      image_props.maxExtent.height < image_info.extent.height ||
      image_props.maxExtent.depth < image_info.extent.depth ||
      image_props.maxMipLevels < image_info.mipLevels ||
      image_props.maxArrayLayers < image_info.arrayLayers) {
    XELOGE(
        ...
        image_props.maxExtent.width, image_props.maxExtent.height,
        image_props.maxExtent.depth, image_props.maxMipLevels,
        image_props.maxArrayLayers, static_cast<uint32_t>(image_info.usage),
        static_cast<uint32_t>(image_info.flags));
    return nullptr;
  }
